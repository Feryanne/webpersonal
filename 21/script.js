...
function startApp() {
  ...
  soundManager.preload().then(init, (reason) => {
    // Jika terjadi error saat preload audio, tetap inisialisasi aplikasi
...
function runCountdown(second) {
  let countdown = seconds;
  const countdownInterval = setInterval(() => {
    setLoadingStatus(`Starting in ${countdown} seconds...`);
    countdown--;

    if (countdown <= 0) {
      clearInterval(countdownInterval);
      startApp(); // Mulai aplikasi setelah countdown selesai
    }
  }, 1000);
}
...
if  (IS_HEADER) {
  ...
  setLoadingStatus("Load for Show");
  setTimeout(() => {
    runCountdown(5); // Countdown 5 detik sebelum aplikasi dimulai
  }, 0);
    soundManager.preload().then(init, (reason) => {
...
function updateCountdown() {
  countdownElement.textContent = countdown; // Tampilkan angka countdown
  if (countdown > 0) {
    countdown--; // Kurangi angka countdown
  } else {
    clearInterval(countdownTimer); // Hentikan timer saat selesai
    loadingScreen.style.display = "none"; // Sembunyikan tampilan loading
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const text2 = "APPRECIATION IS EVERYTHING";
  const text3 = "Don't expect more from anything we do,";
  const typingSpeed = 100; // Kecepatan mengetik (ms)
  const delayBeforeNextText = 3000; // Waktu jeda sebelum teks kedua muncul (3 detik)

  const textElement1 = document.getElementById("typingText1");
  const textElement2 = document.getElementById("typingText2");
  const textElement3 = document.getElementById("typingText3");

  // Fungsi untuk mengetikkan teks
  function typeText(text, element, callback) {
    let i = 0;
    element.textContent = ""; // Reset teks
    element.style.opacity = "1"; // Tampilkan teks
    function typing() {
      if (i < text.length) {
        ...
        setTimeout(callback, delayBeforeNextText);
      }
    }
    typing();
  }

  // Mengetik teks pertama
  typeText(text1, textElement1, () => {
    // Setelah teks pertama selesai, tampilkan teks kedua
    textElement2.style.opacity = "1"; // Tampilkan teks kedua
    typeText(text2, textElement2);
    textElement3.style.opacity = "1"; // Tampilkan teks kedua
    typeText(text3, textElement3);
  });
});
